...
bool CtestLayer::init(){
    ...
    lightSource->setSpecular(cc3dv4(0.4,0.4,0.4,1));
    lightSource->setPositionC3D(cc3dv4(1000,5000,1000,1));//(cc3dv4(600, 900, 1200, 1));
    ...
    //--------------------------------------------------------------------
    //----import _cfc model
    m_skinActor3D=new Cc3dSkinActor();
    m_skinActor3D->autorelease();
    string path="toolKitRes/model/girl_cfc";
    //string path="export/girl_readable_cfc";
    cout<<"loading from "<<path<<endl;
    m_skinActor3D->doImport(path.c_str());
    cout<<"loading done!"<<endl;
    m_skinActor3D->setPassUnifoCallback(passUnifoCallback_classicLighting);
    m_skinActor3D->setProgram(program);
    m_root3d->addChild(m_skinActor3D,0);
    m_skinActor3D->setPositionC3D(Cc3dVector4(0,-170,0,1));
    m_skinActor3D->setScaleC3D(6, 6, 6);
    m_skinActor3D->scheduleUpdate();
    m_skinActor3D->submit(GL_STATIC_DRAW);
...
void CtestLayer::transformCallBack(Ref *senderz, cocos2d::extension::Control::EventType controlEvent){
    if(m_isDoUpdate){
        ...
        m_skinActor3D->setTransformC3D(m_initialMat);
        //stop update
        m_isDoUpdate=false;
        
    }else{
        //store inital matrix
        m_initialMat=m_skinActor3D->getTransformC3D();
        //start update
        m_isDoUpdate=true;
    }
}
void CtestLayer::update(float dt){
    ...
    m_skinActor3D->rotateC3D(cc3dv4(0, 1, 0, 0), 120*dt);
